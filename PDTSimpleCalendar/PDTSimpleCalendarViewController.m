...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    self = [self initWithCollectionViewLayout:[[PDTSimpleCalendarViewFlowLayout alloc] init]];
    if (self) {
...
- (NSDate *)lastDate
{
    if (!_lastDate) {
        ...
        offsetComponents.day = self.lastDateOffsetDay - 1;
        [self setLastDate:[self.calendar dateByAddingComponents:offsetComponents toDate:_notUseFirstDateMonthWhenSetLastDate ? [NSDate date] : self.firstDateMonth options:0]];
    }
...
- (void)viewDidLoad
{
    ...
    self.collectionView.allowsMultipleSelection = self.multiSelectable;
    self.collectionView.allowsSelection = self.selectable;
}
